...
public:
    ...
    void toOrderCommand() {
        if (!timing) {
            std::cout << "Customer 3: I am heading to order counter\n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 0.88) {
            command = "Right";
        } else if ((currentTime - startTime) < 24) {
            command = "Straight";
        }  else if ((currentTime - startTime) < 24.1) {
            command = "Stop";
        }  else {
            emitter->setChannel(5);
            std::cout << "Customer 3: Hi Staff, I want to order " << coffee << " \n";
            std::string message = "3,Customer3," + coffee + ",atOrderTile";
    ...
    void toStartFromOrderCommand() {
        if (!timing) {
            std::cout << "Customer 3: I am returning to starting point \n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 1.99) {
            command = "Right";
    ...
    void toPickupCommand() {
        if (!timing) {
            std::cout << "Customer 3: I am heading to pickup counter\n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 1.7) {
            command = "Left";
        } else if ((currentTime - startTime) < 26) {
            command = "Straight";
        } else if ((currentTime - startTime) < 26.1) {
            command = "Stop";
        }  else {
            std::cout << "Customer 3: I got my " << coffee << " \n";
            toPickup = false;
            timing = false;
    ...
    void toStartfromPickupCommand() {
        ...
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 1.97) {
            command = "Right";
        } else if ((currentTime - startTime) < 25.7) {
            command = "Straight";
        }  else if ((currentTime - startTime) < 26.1) {
            command = "Stop";
        }  else {
            ...
            if (receiver->getQueueLength() > 0) {
                ...
                if (message.find("CheckBalance") != std::string::npos) {
                    ...
                    if (cash >= std::stoi(instructions[0])) {
                        std::cout << "Customer 3: Hi Staff, I will buy it\n";
                        ...
                        std::cout << "Customer 3: Hi Staff, oops, I can't afford it \n";
                        emitter->setChannel(5);
