...
public:
    ...
    void toOrderCommand() {
        if (!timing) {
            std::cout << "Customer 4: I am heading to order counter\n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 1.05) {
            command = "Right";
            ...
            std::cout << "Customer 4: Hi Staff, I want to order " << coffee << " \n";
            std::string message = "4,Customer4," + coffee + ",atOrderTile";
    ...
    void toStartFromOrderCommand() {
        if (!timing) {
            std::cout << "Customer 4: I am returning to starting point \n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
    ...
    void toPickupCommand() {
        if (!timing) {
            std::cout << "Customer 4: I am heading to pickup counter\n";
            startTime = robot.getTime();
            timing = true;
        }
        double currentTime = robot.getTime();
        if ((currentTime - startTime) < 1.7) {
            command = "Left";
        } else if ((currentTime - startTime) < 27.6) {
            command = "Straight";
        } else if ((currentTime - startTime) < 26.1) {
            command = "Stop";
        }  else {
            std::cout << "Customer 4: I got my " << coffee << " \n";
            toPickup = false;
            timing = false;
            ...
            if (receiver->getQueueLength() > 0) {
                ...
                if (message.find("CheckBalance") != std::string::npos) {
                    ...
                    if (cash >= std::stoi(instructions[0])) {
                        std::cout << "Customer 4: Hi Staff, I will buy it\n";
                        ...
                        std::cout << "Customer 4: Hi Staff, oops, I can't afford it \n";
                        emitter->setChannel(5);
